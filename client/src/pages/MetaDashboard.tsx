...
export default function MetaDashboard({ section }: { section: Section }) {
  ...
  if (section === "overview") {
    ...
    body = <div className="space-y-5">{configCard}<Card><CardHeader><CardTitle>כתובת הוובהוק</CardTitle></CardHeader><CardContent className="flex items-center gap-2"><code className="min-w-0 flex-1 truncate rounded bg-muted p-3 text-xs">{callbackUrl}</code><Button variant="outline" size="icon" onClick={() => void navigator.clipboard.writeText(callbackUrl)}><Copy className="h-4 w-4" /></Button></CardContent></Card><Card><CardContent className="p-5 text-sm text-muted-foreground">הזן את ערכי Meta רק באזור הסודות של הפרויקט. הם אינם נשמרים בקוד, במסד הנתונים או בממשק.</CardContent></Card></div>;
  }
